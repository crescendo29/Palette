...
const currentHexes =
	document.querySelectorAll(".color h2");
...
function updateTextUI(index) {
	...
	const icons = activeDiv.querySelectorAll(
		".controls button"
	);
...
function resetInputs() {
	...
	sliders.forEach((slider) => {
		...
		if (slider.name === "brightness") {
			...
			const brightValue =
				chroma(brightColor).hsl()[2];
			slider.value =
				Math.floor(brightValue * 100) / 100;
		}
...
function openAdjustmentPanel(index) {
	sliderContainers[index].classList.toggle(
		"active"
	);
}
...
function lockLayer(e, index) {
	...
	if (
		lockSVG.classList.contains("fa-lock-open")
	) {
		e.target.innerHTML =
...
const submitSave = document.querySelector(
	".submit-save"
);
const closeSave = document.querySelector(
	".close-save"
);
const saveContainer = document.querySelector(
	".save-container"
);
const saveInput = document.querySelector(
...
function savePalette(e) {
	...
	let paletteNr;
	const paletteObjects = JSON.parse(
	...
	paletteBtn.addEventListener("click", (e) => {
		...
		initialColors = [];
		savedPalettes[paletteIndex].colors.forEach(
			(color, index) => {
	...
	libraryContainer.children[0].appendChild(
		palette
	);
}
